...
def update_resume(
    *,
    ...
    if "personal_information" in update_dict:
        update_dict["personal_information"] = resume_in.personal_information.model_dump()
    ...
    if "certifications" in update_dict:
        update_dict["certifications"] = [c.model_dump() for c in resume_in.certifications]
    
    if "languages" in update_dict:
        update_dict["languages"] = [l.model_dump() for l in resume_in.languages]
...
def create_user_default_config(
...
def create_user_default_resume(
    user_id: UUID, session: Session
) -> PlainTextResume:
